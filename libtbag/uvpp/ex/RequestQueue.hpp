...
public:
    inline bool getPrepareEmpty() const TBAG_NOEXCEPT_SP_OP(_prepare.empty())
    { Guard g(_mutex); return _prepare.empty(); }
    inline bool getActiveEmpty() const TBAG_NOEXCEPT_SP_OP(_active.empty())
    { Guard g(_mutex); return _active.empty(); }

    inline std::size_t getPrepareSize() const TBAG_NOEXCEPT_SP_OP(_prepare.size())
    { Guard g(_mutex); return _prepare.size(); }
    inline std::size_t getActiveSize() const TBAG_NOEXCEPT_SP_OP(_active.size())
