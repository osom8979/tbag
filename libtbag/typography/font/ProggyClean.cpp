/**
 * @file   ProggyClean.cpp
 * @brief  ProggyClean class implementation.
 * @author zer0
 * @date   2017-11-18
 */
...
{
    return __impl::PROGGY_CLEAN_FONT_ZIP_AND_BASE64;
}

std::string getProggyCleanMd5()
{
    return __impl::PROGGY_CLEAN_FONT_MD5;
}

util::Buffer getProggyClean()
{
    util::Buffer result;
    if (archive::ex::decodeZipBase64(getCompressedProggyClean(), result)) {
        return result;
    }
    return util::Buffer();
}
