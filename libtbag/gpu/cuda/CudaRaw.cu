...
tbBOOL tbCudaAdd1f(float const * gpu_v1, float const * gpu_v2, float * gpu_result, int size, void * stream)
{
    int block_size    = 0;
    int min_grid_size = 0;
    int grid_size     = 0;
...
tbBOOL tbCudaAdd1d(double const * gpu_v1, double const * gpu_v2, double * gpu_result, int size, void * stream)
{
    int block_size    = 0;
    int min_grid_size = 0;
    int grid_size     = 0;
    ...
    tbCudaAddKernel<double><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(gpu_v1, gpu_v2, gpu_result, size);
    return TB_TRUE;
}
