...
// Kernels.
public:
    virtual Err fill(GpuStream const & stream, int      * out, int      data, int count, GpuEvent * event) const { return Err::E_UNSUPOP; }
    virtual Err fill(GpuStream const & stream, unsigned * out, unsigned data, int count, GpuEvent * event) const { return Err::E_UNSUPOP; }
