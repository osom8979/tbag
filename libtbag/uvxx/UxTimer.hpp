...
public:
    UxTimer();
    UxTimer(std::nullptr_t) TBAG_NOEXCEPT;
...
public:
    inline friend void swap(UxTimer & lh, UxTimer & rh) TBAG_NOEXCEPT { lh.swap(rh); }
...
public:
     ...
     * @see std::less
     */
    friend inline bool operator <(UxTimer const & x, UxTimer const & y) TBAG_NOEXCEPT
    {
        return x.get() < y.get();
    }

    inline bool operator ==(UxTimer const & obj) const TBAG_NOEXCEPT
    {
        return get() == obj.get();
    }
