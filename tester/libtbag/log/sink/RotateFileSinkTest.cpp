...
TEST(RotateFileSinkTest, Default)
{
    ...
    ASSERT_TRUE(sink.write(0, "1234", 4));
    files = DIR.scanNameOnly();
    std::sort(files.begin(), files.end());
    ...
    ASSERT_TRUE(sink.write(0, "12", 2));
    files = DIR.scanNameOnly();
    std::sort(files.begin(), files.end());
    ASSERT_EQ(3, files.size());

    ASSERT_TRUE(sink.write(0, "3", 1));
    files = DIR.scanNameOnly();
    std::sort(files.begin(), files.end());
    ASSERT_EQ(3, files.size());
    ...
    ASSERT_TRUE(sink.write(0, "12341234", 8));
    files = DIR.scanNameOnly();
    std::sort(files.begin(), files.end());
